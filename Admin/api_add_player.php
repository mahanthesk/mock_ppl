<?php
// Admin/api_add_player.php - Single API endpoint to handle both player registration and player statistics in a transaction
...
try {
    $sql = "INSERT INTO registrations (
        ...
        jersey_size, track_pant_size, team_id, created_at
    ) VALUES (
        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW()
    )";
...
if ($add_stats_now) {
    ...
    try {
        $statsSql = "INSERT INTO players_stats (
            player_id, match_name, opponent, tournament, ground, match_date,
        ...
        $statsStmt->execute([
            ...
            trim($_POST['opponent'] ?? '') !== '' ? trim($_POST['opponent']) : null,
            trim($_POST['tournament'] ?? 'PPL'),
            trim($_POST['ground'] ?? '') !== '' ? trim($_POST['ground']) : null,
            !empty($_POST['match_date']) ? $_POST['match_date'] : null,
            intval($_POST['runs'] ?? 0),
        ...
        try {
            ...
            $aggStmt->execute([
                $new_player_id, $r, $r, $r, $sr, $w, $b_avg, $eco, $c, $st
            ]);
        } catch (Exception $e) {
