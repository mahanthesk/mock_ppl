...
// Fetch counts
function getCounts($pdo, $column)
{
    $stmt = $pdo->prepare("SELECT $column, COUNT(*) as count FROM registrations WHERE deleted_at IS NULL GROUP BY $column");
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

$categories = getCounts($pdo, 'category');
$roles = getCounts($pdo, 'role');
$battingTypes = getCounts($pdo, 'batting_type');
$bowlingTypes = getCounts($pdo, 'bowling_type');
...
foreach ($categories as $cat) {
    $totalPlayers += $cat['count'];
}

// Calculate grouped role counts
$roleCounts = [
    'Batsman' => 0,
    'Bowler' => 0,
    'All-rounder' => 0,
...
foreach ($roles as $r) {
    $roleName = strtolower(trim($r['role']));
    if (strpos($roleName, 'bat') !== false) {
        $roleCounts['Batsman'] += $r['count'];
    } elseif (strpos($roleName, 'bowl') !== false) {
        $roleCounts['Bowler'] += $r['count'];
    } elseif (strpos($roleName, 'all') !== false) {
        $roleCounts['All-rounder'] += $r['count'];
    } elseif (strpos($roleName, 'wicket') !== false) {
        $roleCounts['Wicketkeeper'] += $r['count'];
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Admin Dashboard - PPL</title>
    ...
    <style>
        ...
        .stat-card {
            ...
            box-shadow: 0 8px 25px rgba(15, 42, 74, 0.06) !important;
            padding: 24px;
            height: 100%;
        ...
        .stat-card:hover {
            ...
            box-shadow: 0 16px 35px rgba(15, 42, 74, 0.12) !important;
        ...
        <div class="row">
            ...
            <div class="col-lg-10 col-md-9 dashboard-container">
                <div class="row mb-4 align-items-center">
                    <div class="col-md-8">
                        <h2 class="fw-bold mb-0" style="font-family: 'Poppins', sans-serif; color: #0f2a4a; letter-spacing: -0.5px;">Dashboard <span style="color: #0284c7;">Overview</span></h2>
                    </div>
                ...
                <div class="row mb-4 g-3">
                    <div class="col">
                        <div class="stat-card text-center h-100" style="cursor: default; padding: 15px;">
                            <div class="stat-title" style="font-size: 0.9rem; margin-bottom: 10px;">Total Registrations</div>
                            <span class="total-badge" style="font-size: 2rem; margin-top: 5px;">
                                <?php echo $totalPlayers; ?>
                            </span>
                        </div>
                    </div>
                    <div class="col">
                        <a href="admin_players.php?role=bat" class="text-decoration-none">
                            <div class="stat-card text-center h-100" style="cursor: pointer; padding: 15px;">
                                <div class="stat-title" style="font-size: 0.9rem; margin-bottom: 10px;">Batsman</div>
                                <span class="total-badge" style="font-size: 2rem; margin-top: 5px;">
                                    <?php echo $roleCounts['Batsman']; ?>
                                </span>
                            </div>
                        </a>
                    </div>
                    <div class="col">
                        <a href="admin_players.php?role=bowl" class="text-decoration-none">
                            <div class="stat-card text-center h-100" style="cursor: pointer; padding: 15px;">
                                <div class="stat-title" style="font-size: 0.9rem; margin-bottom: 10px;">Bowler</div>
                                <span class="total-badge" style="font-size: 2rem; margin-top: 5px;">
                                    <?php echo $roleCounts['Bowler']; ?>
                                </span>
                            </div>
                        </a>
                    </div>
                    <div class="col">
                        <a href="admin_players.php?role=all" class="text-decoration-none">
                            <div class="stat-card text-center h-100" style="cursor: pointer; padding: 15px;">
                                <div class="stat-title" style="font-size: 0.9rem; margin-bottom: 10px;">All Rounder</div>
                                <span class="total-badge" style="font-size: 2rem; margin-top: 5px;">
                                    <?php echo $roleCounts['All-rounder']; ?>
                                </span>
                            </div>
                        </a>
                    </div>
                    <div class="col">
                        <a href="admin_players.php?role=wicket" class="text-decoration-none">
                            <div class="stat-card text-center h-100" style="cursor: pointer; padding: 15px;">
                                <div class="stat-title" style="font-size: 0.9rem; margin-bottom: 10px;">Wicket Keeper</div>
                                <span class="total-badge" style="font-size: 2rem; margin-top: 5px;">
                                    <?php echo $roleCounts['Wicketkeeper']; ?>
                                </span>
                            </div>
                        </a>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-4">
                        <div class="stat-card">
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <div class="stat-item">
                                    <span>
                                        <?php echo $cat['category'] ?: 'Unspecified'; ?>
                                    </span>
                    ...
                    <div class="col-md-3 mb-4">
                        <div class="stat-card">
                            <div class="stat-title">Player Roles</div>
                            <?php foreach ($roles as $role): ?>
                                <div class="stat-item">
                                    <span>
                                        <?php echo $role['role'] ?: 'Unspecified'; ?>
                                    </span>
                                    <span class="stat-value">
                                        <?php echo $role['count']; ?>
                                    </span>
                    ...
                    <div class="col-md-3 mb-4">
                        <div class="stat-card">
                            <div class="stat-title">Batting Style</div>
                            <?php foreach ($battingTypes as $type): ?>
                                <div class="stat-item">
                                    <span>
                                        <?php echo $type['batting_type'] ?: 'Unspecified'; ?>
                                    </span>
                                    <span class="stat-value">
                                        <?php echo $type['count']; ?>
                                    </span>
                    ...
                    <div class="col-md-3 mb-4">
                        <div class="stat-card">
                            <div class="stat-title">Bowling Style</div>
                            <?php foreach ($bowlingTypes as $type): ?>
                                <div class="stat-item">
                                    <span>
                                        <?php echo $type['bowling_type'] ?: 'Unspecified'; ?>
                                    </span>
                                    <span class="stat-value">
                                        <?php echo $type['count']; ?>
                                    </span>
                        ...
                        <div id="statisticsSectionContainer">
                            ...
                            <div class="custom-section-card">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-2">
                                        <label class="form-label small text-muted">50s</label>
                                        <input type="number" name="fifties" class="form-control custom-input" min="0" value="0">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="form-label small text-muted">100s</label>
                                        <input type="number" name="hundreds" class="form-control custom-input" min="0" value="0">
                                    </div>
                                    <div class="col-md-4">
